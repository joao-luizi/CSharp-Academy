...
internal class GameRun : State
{
    ...
    public override void Print()
    {
        ...
        Console.Clear();
        Console.WriteLine($"Timer: {timeCounter.ToString().PadLeft(4, '0')}");
        Console.WriteLine(Helpers.OperationTitle(op.Operators)); // Title
        Console.WriteLine($"Problem set {operationIndex + 1} of {operations.Count}. Run Score is: {RunScore}.");
        Console.WriteLine($"Question: {op.GetUserExpression()} ?");
    ...
    public override State? Process()
    {
        ...
        result = Helpers.ValidateInput(_context._settings.Difficulty, input);

        timer.Stop();
        timer.Dispose();
        if (timeCounter == 0)
        {
            Console.WriteLine($"You ran out of time. The solution was {op.OperationResult}. Type any key to continue.");
        }
        else
        {
            ...
            {
                Console.WriteLine($"Your answer is incorrect. The solution was {op.OperationResult}. Type any key to continue.");
            }
        }
        operationIndex++;
        Console.ReadLine();
